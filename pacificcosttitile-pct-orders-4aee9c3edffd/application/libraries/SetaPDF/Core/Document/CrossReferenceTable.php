 ...
 * @version    $Id: CrossReferenceTable.php 298 2012-10-31 14:36:24Z maximilian $
 */

/**
 * Class representing a crossreference table
 *
 * @copyright  Copyright (c) 2012 Setasign - Jan Slabon (http://www.setasign.de)
 * @category   SetaPDF
 * @package    SetaPDF_Core
...
class SetaPDF_Core_Document_CrossReferenceTable
{
    ...
    )
    {
        ...
        foreach ($offsets AS $objectId => $generation) {
            ...
            if ($lastObjId !== null && $objectId != ($lastObjId + 1)) {
                if (true === $onlyUpdated && $lastObjId !== 0) {
                    ...
                    while (($lastObjId + 1) < $objectId) {
                        $entrieLengths[2] = max(2, $entrieLengths[2]);
            ...
            $entrieLengths[0] = max(strlen($type), $entrieLengths[0]);

            $offset = ltrim(pack("N", $data[0]), "\0");
            $entrieLengths[1] = max(strlen($offset), $entrieLengths[1]);

            $generation = ltrim(pack("N", $data[1]), "\0");
            $entrieLengths[2] = max(strlen($generation), $entrieLengths[2]);
        ...
        $value['W'] = new SetaPDF_Core_Type_Array(array(
            new SetaPDF_Core_Type_Numeric($entrieLengths[0]),
            new SetaPDF_Core_Type_Numeric($entrieLengths[1]),
            new SetaPDF_Core_Type_Numeric($entrieLengths[2]),
        ));
        ...
        foreach ($sections AS $section) {
            ...
            foreach ($section AS $entry) {
                if ($entrieLengths[0] > 0)
                    $entry[0] = str_pad($entry[0], $entrieLengths[0], "\0", STR_PAD_LEFT);
                if ($entrieLengths[1] > 0)
                    $entry[1] = str_pad($entry[1], $entrieLengths[1], "\0", STR_PAD_LEFT);
                if ($entrieLengths[2] > 0)
                    $entry[2] = str_pad($entry[2], $entrieLengths[2], "\0", STR_PAD_LEFT);
    ...
    (
        SetaPDF_Core_Writer_Interface $writer,
        $onlyUpdated = true
    )
    {
        if (count($this->_updatedOffsets) === 0 && true == $onlyUpdated) {
