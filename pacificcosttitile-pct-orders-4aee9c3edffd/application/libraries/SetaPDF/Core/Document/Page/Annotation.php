 ...
 * @version    $Id: Annotation.php 520 2013-08-12 14:38:45Z jan.slabon $
 */
...
class SetaPDF_Core_Document_Page_Annotation
{
     ...
     */
    static protected function _createAnnotationDictionary(SetaPDF_Core_DataStructure_Rectangle $rect, $subtype)
    {
     ...
     * 
     * @param SetaPDF_Core_Type_Abstract|SetaPDF_Core_Type_Dictionary|SetaPDF_Core_Type_IndirectObjectInterface $objectOrDictionary
     * @throws InvalidArgumentException
     * @return SetaPDF_Core_Document_Page_Annotation
     */
    static public function byObjectOrDictionary(SetaPDF_Core_Type_Abstract $objectOrDictionary)
    {
    ...
    public function getIndirectObject(SetaPDF_Core_Document $document = null)
    {
        if ($this->_indirectReference === null && $document !== null) {
            $this->_indirectReference = $document->createNewObject($this->getAnnotationDictionary());
        }
    ...
    {
        return $this->getAnnotationDictionary()->getValue('Subtype')->ensure()->getValue();
    }
    
    /**
     * Get the rectange object of this annotation
     * 
    ...
    {
        if (null === $this->_rect) {
            $this->_rect = new SetaPDF_Core_DataStructure_Rectangle($this->_annotationDictionary->getValue('Rect')->ensure());
        }
    ...
    {
        if (!$this->_annotationDictionary->offsetExists('NM'))
            return null;

        return SetaPDF_Core_Encoding::convertPdfString(
            $this->_annotationDictionary->getValue('NM')->getValue(), $encoding
        );
    }
    ...
    {
        if (null === $name) {
            $this->_annotationDictionary->offsetUnset('NM');
            return;
        }
        ...
        if (!$this->_annotationDictionary->offsetExists('NM')) {
            $this->_annotationDictionary->offsetSet('NM', new SetaPDF_Core_Type_String($name));
            return;
        }

        $this->_annotationDictionary->getValue('NM')->setValue($name);
    }
    ...
    {
        if (!$this->_annotationDictionary->offsetExists('Contents'))
            return null;

        return SetaPDF_Core_Encoding::convertPdfString(
            $this->_annotationDictionary->getValue('Contents')->getValue(), $encoding
        );
    }

    /**
     * Set teh contents of the annotation
     *
    ...
    {
        if (null === $contents) {
            $this->_annotationDictionary->offsetUnset('Contents');
            return;
        }
        ...
        if (!$this->_annotationDictionary->offsetExists('Contents')) {
            $this->_annotationDictionary->offsetSet('Contents', new SetaPDF_Core_Type_String($contents));
            return;
        }

        $this->_annotationDictionary->getValue('Contents')->setValue($contents);
    }

    /**
     * Get the modifiation date
     *
     * @param $asString
     * @return mixed|null|SetaPDF_Core_DataStructure_Date
     */
    public function getModificationDate($asString)
    {
        if (!$this->_annotationDictionary->offsetExists('M'))
            return null;

        $date = $this->_annotationDictionary->getValue('M')->ensure()->getValue();
        if (true === $asString) {
            return $date;
        }
    ...
    {
        if ($date === null) {
            $this->_annotationDictionary->offsetUnset('M');
            return;
        }
        ...
        $this->_annotationDictionary->offsetSet('M', $date->getValue());
    }
  ...
  /* Annotation Flags (F entry) */
    ...
    public function setAnnotationFlags($flags, $set = true)
    {
    	...
    	$dict = $this->getAnnotationDictionary();
    ...
    {
        $dictionary = $this->getAnnotationDictionary();
     ...
     *
     * @param SetaPDF_Core_DataStructure_Color|scalar|array $color
     */
    public function setColor($color)
    {
        if (null === $color) {
            $this->_annotationDictionary->offsetUnset('C');
            return;
        }
        ...
        $this->_annotationDictionary->offsetSet('C', $color->getValue());
    }
    ...
    {
        if (!$this->_annotationDictionary->offsetExists('C'))
            return null;

        return SetaPDF_Core_DataStructure_Color::createByComponents(
            $this->_annotationDictionary->getValue('C')
        );
    }
    ...
    {
        if (!$this->_annotationDictionary->offsetExists('AP')) {
            return null;
        }

        $ap = $this->_annotationDictionary->getValue('AP')->ensure();
    ...
    {
        if (!$this->_annotationDictionary->offsetExists('AP')) {
            $this->_annotationDictionary->offsetSet('AP', new SetaPDF_Core_Type_Dictionary());
        }
