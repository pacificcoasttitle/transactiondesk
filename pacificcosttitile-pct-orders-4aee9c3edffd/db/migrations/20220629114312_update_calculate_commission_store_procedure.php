...
final class UpdateCalculateCommissionStoreProcedure extends AbstractMigration
{
    /**
     * Change Method.
     *
    ...
    public function change(): void
    {
		...
		DECLARE total_commisssion,temp_commission,commission_draw FLOAT DEFAULT 0;
		...
		DECLARE order_details_cur
				CURSOR FOR 
					SELECT order_details.prod_type,order_details.underwriter,order_details.premium,REPLACE(transaction_details.loan_amount, ',', '') as loan_amount,REPLACE(transaction_details.sales_amount, ',', '') as sales_amount,customer_basic_details.commission_draw_value
					FROM order_details
		...
		CREATE TEMPORARY TABLE my_temp_table (tier_id_temp INT,premium_temp FLOAT,commission_temp FLOAT,prod_type_temp VARCHAR(10),underwriter_temp VARCHAR(20),check_amount_temp FLOAT,added_from SMALLINT);
				...
				get_details : LOOP FETCH order_details_cur INTO prod_type, underwriter,premium,loan_amount,sales_amount,commission_draw;
