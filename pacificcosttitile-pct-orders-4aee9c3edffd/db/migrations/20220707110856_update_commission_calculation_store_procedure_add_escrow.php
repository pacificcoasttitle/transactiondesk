...
final class UpdateCommissionCalculationStoreProcedureAddEscrow extends AbstractMigration
{
    /**
     * Change Method.
     *
    ...
    public function change(): void
    {
		...
		DECLARE prod_type,underwriter,sales_amount,loan_amount,check_amount,prod_type_temp,underwriter_temp,file_type_name  CHAR(255);
		...
		DECLARE allow_threshold,escrow_threshold TINYINT(1);
		DECLARE total_commisssion,temp_commission,commission_draw,escrow_commission,escrow_commission_total,escrow_premium,settlement_fees FLOAT DEFAULT 0;
		...
		DECLARE order_details_cur
				CURSOR FOR 
					SELECT order_details.prod_type,order_details.underwriter,order_details.premium,REPLACE(transaction_details.loan_amount, ',', '') as loan_amount,REPLACE(transaction_details.sales_amount, ',', '') as sales_amount,customer_basic_details.commission_draw_value,pct_order_product_types.product_type as file_type_name_tbl,order_details.escrow_amount
					FROM order_details
					...
					JOIN pct_order_product_types ON transaction_details.purchase_type = pct_order_product_types.product_type_id AND pct_order_product_types.status=1
		...
		CREATE TEMPORARY TABLE my_temp_table (tier_id_temp INT,premium_temp FLOAT,commission_temp FLOAT,prod_type_temp VARCHAR(10),underwriter_temp VARCHAR(20),check_amount_temp FLOAT,added_from SMALLINT);
				...
				get_details : LOOP FETCH order_details_cur INTO prod_type, underwriter,premium,loan_amount,sales_amount,commission_draw,file_type_name,settlement_fees;
                ...
                IF file_type_name = TRIM('Loan:  Title and Escrow') AND settlement_fees > 0 THEN
				...
				IF escrow_premium > 0 THEN
					BLOCK7 : BEGIN
						...
						FETCH user_override_cur INTO underwriter_users_id,allow_threshold,fix_commsision;
							...
							SET json_obj = JSON_OBJECT('prod_type','escrow','commisison',temp_commission);
							SET json_arr =  JSON_ARRAY_APPEND(json_arr, '$', json_obj);
							ELSE 
							IF allow_threshold = 1 THEN
