...
final class UpdateCalculateCommissionsTriggers extends AbstractMigration
{
    /**
     * Change Method.
     *
    ...
    public function change(): void
    {
		...
		$order_details_update = "DROP TRIGGER IF EXISTS `order_details_after_update`;
			...
			IF (OLD.premium <> NEW.premium OR OLD.sent_to_accounting_date <> NEW.sent_to_accounting_date OR OLD.prod_type <> NEW.prod_type OR OLD.underwriter <> NEW.underwriter OR OLD.escrow_amount <> NEW.escrow_amount OR OLD.loan_amount <> NEW.loan_amount OR OLD.sales_amount <> NEW.sales_amount) AND (MONTH(OLD.sent_to_accounting_date) =  for_month AND YEAR(OLD.sent_to_accounting_date) = for_year) THEN
			BLOCK1: BEGIN
				DECLARE get_sales_rep_cur
					CURSOR FOR 
						SELECT transaction_details.sales_representative
