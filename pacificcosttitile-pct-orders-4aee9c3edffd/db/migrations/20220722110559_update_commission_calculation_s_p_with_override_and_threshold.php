...
final class UpdateCommissionCalculationSPWithOverrideAndThreshold extends AbstractMigration
{
    /**
     * Change Method.
     *
    ...
    public function change(): void
    {
		...
		DECLARE done,tier_done,override_done,threshold_done,commission_calculated,is_first_record,override_user_add_tmp,override_user_sub_tmp INT DEFAULT 0;
		...
		DECLARE total_commisssion,temp_commission,commission_draw,commission_first_threshold,escrow_commission,escrow_commission_total,escrow_premium,settlement_fees,settlement_fees_loan,settlement_fees_sale,override_comm_add_tmp,override_comm_sub_tmp,override_comm_add_tmp_cal FLOAT DEFAULT 0;
		...
		DECLARE order_details_cur
				CURSOR FOR 
					SELECT order_details.prod_type,order_details.underwriter,order_details.premium,CASE WHEN order_details.loan_amount IS NULL THEN REPLACE(transaction_details.loan_amount, ',', '') ELSE REPLACE(order_details.loan_amount, ',', '') END as loan_amount,CASE WHEN order_details.sales_amount IS NULL THEN REPLACE(transaction_details.sales_amount, ',', '') ELSE REPLACE(order_details.sales_amount, ',', '') END as sales_amount,customer_basic_details.commission_draw_value,order_details.escrow_amount,so1.override_user_id as override_user_add,so1.commission as override_comm_add,so2.user_id as override_user_sub,so2.commission as override_comm_sub,first_in_threshold
					FROM order_details
					...
					LEFT JOIN sales_rep_commission_override so1 ON so1.user_id = user_id
					LEFT JOIN sales_rep_commission_override so2 ON so2.override_user_id = user_id
		...
		CREATE TEMPORARY TABLE my_temp_table (tier_id_temp INT,premium_temp FLOAT,commission_temp FLOAT,prod_type_temp VARCHAR(10),underwriter_temp VARCHAR(20),check_amount_temp FLOAT,added_from SMALLINT);
				...
				get_details : LOOP FETCH order_details_cur INTO prod_type, underwriter,premium,loan_amount,sales_amount,commission_draw,settlement_fees,override_user_add_tmp,override_comm_add_tmp,override_user_sub_tmp,override_comm_sub_tmp,commission_first_threshold;
			...
			BLOCK4: BEGIN
                ...
                IF total_commisssion > 0 THEN
                    ...
                    SET json_arr =  JSON_ARRAY_APPEND(json_arr, '$', json_obj);
					IF total_commisssion > 0 THEN
						IF override_comm_sub_tmp > 0 AND override_user_sub_tmp > 0 THEN
							SET temp_commission = ((total_commisssion * override_comm_sub_tmp) / 100);
							SET json_obj = JSON_OBJECT('prod_type','override_sub','override_id',override_user_sub_tmp,'commisison',temp_commission);
							SET json_arr =  JSON_ARRAY_APPEND(json_arr, '$', json_obj);
						END IF;
					END IF;
				...
				IF override_comm_add_tmp > 0 AND override_user_add_tmp > 0 THEN
					CALL `calculate_commission`(override_user_add_tmp);
					SELECT umc.commission INTO override_comm_add_tmp_cal FROM pct_user_monthly_commission umc WHERE umc.user_id = override_user_add_tmp AND umc.commission_month = for_month AND umc.commission_year = for_year;
					IF override_comm_add_tmp_cal > 0 THEN
						SET temp_commission = ((override_comm_add_tmp_cal * override_comm_add_tmp) / 100);
						SET json_obj = JSON_OBJECT('prod_type','override_add','override_id',override_user_add_tmp,'commisison',temp_commission);
						SET json_arr =  JSON_ARRAY_APPEND(json_arr, '$', json_obj);
					END IF;
				END IF;
