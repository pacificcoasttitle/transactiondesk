...
final class UpdateCommissionCalculationSPWithUserCondition extends AbstractMigration
{
    /**
     * Change Method.
     *
    ...
    public function change(): void
    {
		...
		DECLARE allow_threshold,escrow_threshold,loop_x,apply_bonus,user_available TINYINT(1);
		DECLARE total_commisssion,temp_commission,commission_draw,commission_first_threshold,escrow_commission,escrow_commission_total,escrow_premium,settlement_fees,settlement_fees_loan,settlement_fees_sale,bonus_amount,premium_for_bonus FLOAT DEFAULT 0;
		...
		DECLARE order_details_cur
				CURSOR FOR 
					SELECT order_details.prod_type,order_details.underwriter,order_details.premium,CASE WHEN order_details.loan_amount IS NULL THEN REPLACE(transaction_details.loan_amount, ',', '') ELSE REPLACE(order_details.loan_amount, ',', '') END as loan_amount,CASE WHEN order_details.sales_amount IS NULL THEN REPLACE(transaction_details.sales_amount, ',', '') ELSE REPLACE(order_details.sales_amount, ',', '') END as sales_amount,customer_basic_details.commission_draw_value,order_details.escrow_amount,first_in_threshold,customer_basic_details.apply_bonus
					FROM order_details
		...
		CREATE TEMPORARY TABLE my_temp_table (tier_id_temp INT,premium_temp FLOAT,commission_temp FLOAT,prod_type_temp VARCHAR(10),underwriter_temp VARCHAR(20),check_amount_temp FLOAT,added_from SMALLINT);
				...
				get_details : LOOP FETCH order_details_cur INTO prod_type, underwriter,premium,loan_amount,sales_amount,commission_draw,settlement_fees,commission_first_threshold,apply_bonus;
				 
					IF done = 1 THEN
					LEAVE get_details;
				END IF;

				SET user_available = 1;
                ...
                -- Get Total Premium to calculate Bonus
                SET premium_for_bonus = premium_for_bonus + premium;
			...
			BLOCK4: BEGIN
				...
				-- ESCROW COMMISSION
                -- BONUS
                IF apply_bonus = 1 AND premium_for_bonus > 0 THEN

                    SELECT pct_commission_bonus.bonus_amount
                    INTO bonus_amount
                    FROM pct_commission_bonus
                    WHERE pct_commission_bonus.min_range <= premium_for_bonus
                    ORDER BY pct_commission_bonus.min_range DESC
                    LIMIT 1;
                    IF bonus_amount > 0 THEN
						SET total_commisssion = total_commisssion + bonus_amount;
                        SET json_obj = JSON_OBJECT('prod_type','bonus','commisison',bonus_amount,'premium',premium_for_bonus);
                        SET json_arr =  JSON_ARRAY_APPEND(json_arr, '$', json_obj);
                    END IF;
                END IF;
                -- BONUS
